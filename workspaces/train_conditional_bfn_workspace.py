"""Training Workspace for Conditional BFN Policy.

This workspace handles training of the ConditionalBFNPolicy which uses
an integrated MLP backbone with FiLM/AdaLN conditioning. It follows
the same structure as the diffusion policy workspace.

Features:
- Support for both image and low-dim observations
- Configurable conditioning type (FiLM, AdaLN, Concat)
- EMA model tracking
...
__all__ = ["TrainConditionalBFNWorkspace"]


OmegaConf.register_new_resolver("eval", eval, replace=True)


class TrainConditionalBFNWorkspace(BaseWorkspace):
    """Workspace for training ConditionalBFNPolicy.
    
    ConditionalBFNPolicy uses an integrated backbone (IntegratedBFNBackbone)
    that supports multiple conditioning types:
    ...
    - Training with BFN loss (continuous-time)
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # Compute loss (BFN continuous-time loss)
...
def _main(cfg):
    workspace = TrainConditionalBFNWorkspace(cfg)
    workspace.run()
...
if HAS_HYDRA:
    @hydra.main(
        ...
        config_name="train_conditional_bfn"
    )
    def main(cfg):
        _main(cfg)
else:

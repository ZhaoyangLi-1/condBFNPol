"""Conditional BFN U-Net Hybrid Image Policy.

This module implements a Conditional Bayesian Flow Network policy for visual 
...
- Continuous BFN with integrated conditioning for generative modeling
"""
...
__all__ = ["ConditionalBFNUnetHybridImagePolicy", "BFNConfig"]
...
class ConditionalBFNUnetHybridImagePolicy(BasePolicy):
    """Conditional BFN-based policy for image observations using U-Net backbone.
    ...
    - Uses Continuous BFN with classifier-free guidance support
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f"Conditional BFN U-Net params: {sum(p.numel() for p in self.model.parameters()):.2e}")
    ...
    def predict_action(
        ...
        """Predict action from observation using BFN sampling with optional CFG.
        ...
        if cfg.cond_scale is not None and cfg.cond_scale != 1.0:
            naction = self._sample_bfn_cfg(B, T * Da, cond, device, dtype, cfg.sigma_1, cfg.n_timesteps, cfg.cond_scale)
        else:
            naction = self._sample_bfn(B, T * Da, cond, device, dtype, cfg.sigma_1, cfg.n_timesteps)
    ...
    def _sample_bfn_cfg(
        ...
        for i in range(1, n_steps + 1):
            ...
            x_cond = self.unet_wrapper(mu, t_batch, cond=cond)
            x_uncond = self.unet_wrapper(mu, t_batch, cond=null_cond)
        ...
        # Final with CFG
        t_final = torch.ones(batch_size, device=device, dtype=dtype)
        x_cond = self.unet_wrapper(mu, t_final, cond=cond)
        x_uncond = self.unet_wrapper(mu, t_final, cond=null_cond)
